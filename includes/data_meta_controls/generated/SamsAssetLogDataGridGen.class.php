<?php
	 ...
	 * listing a collection of SamsAssetLog objects.  It includes
	 ...
	 * 
	 */
	class SamsAssetLogDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this SamsAssetLog datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the SamsAssetLog object, or it can be any QQNode extending from QQN::SamsAssetLog().
		 * 
		 ...
		 * sorting using two fields from the SamsAssetLog object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from SamsAssetLog
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intAssetLogId=<?=urlencode($_ITEM->AssetLogId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->AssetLogId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->AssetLogId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = SamsAssetLog::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from SamsAssetLog, given the clauses above
			$this->DataSource = SamsAssetLog::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a SamsAssetLog-based QQNode.
		 * It will also verify that it is a proper SamsAssetLog-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'sams_asset_log') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "sams_asset_log".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'AssetLogId': return QQN::SamsAssetLog()->AssetLogId;
				case 'AssetId': return QQN::SamsAssetLog()->AssetId;
				case 'Asset': return QQN::SamsAssetLog()->Asset;
				case 'CreatedBy': return QQN::SamsAssetLog()->CreatedBy;
				case 'CreatedByObject': return QQN::SamsAssetLog()->CreatedByObject;
				case 'CreationDate': return QQN::SamsAssetLog()->CreationDate;
				case 'LogQtypeId': return QQN::SamsAssetLog()->LogQtypeId;
				case 'Note': return QQN::SamsAssetLog()->Note;
				default: throw new QCallerException('Simple Property not found in SamsAssetLogDataGrid content: ' . $mixContent);
