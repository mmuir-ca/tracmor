...
class ReceiptEditForm extends ReceiptEditFormBase {
	...
	protected function dtgAssetTransact_Create() {
		...
		$this->dtgAssetTransact->AddColumn ( new QDataGridColumn ( 'Status', '<?= $_ITEM->__toStringStatus() ?>', array (
				'CssClass' => "dtg_column",
				'HtmlEntities' => false 
		) ) );
	...
	protected function dtgInventoryTransact_Create() {
		...
		$this->dtgInventoryTransact->AddColumn ( new QDataGridColumn ( 'Status', '<?= $_ITEM->__toStringStatus() ?>', array (
				'CssClass' => "dtg_column",
				'HtmlEntities' => false 
		) ) );
			...
			if ($strAssetCode) {
				...
				if (! $blnError) {
					...
					if (! $blnError) {
						...
						$arrPendingReceipts = AssetTransaction::QueryArray ( QQ::AndCondition ( QQ::Equal ( QQN::AssetTransaction ()->AssetId, $objNewAsset->AssetId ), QQ::In ( QQN::AssetTransaction ()->SourceLocationId, array (
								5,
								2 
						) ), QQ::IsNull ( QQN::AssetTransaction ()->DestinationLocationId ), QQ::NotEqual ( QQN::AssetTransaction ()->TransactionId, $this->objReceipt->TransactionId ) ) );
					...
					if ($objAssetTransaction->AssetTransactionId == $intAssetTransactionId) {
						...
						if ($lstLocationAssetReceived && $lstLocationAssetReceived->SelectedValue) {
							if ($objAssetTransaction->Asset->LinkedFlag) {
								...
								$objAssetTransaction->Asset->Save ();
								$strQuery = sprintf ( "UPDATE  `asset_custom_field_helper` SET  `cfv_11` =  'No', `cfv_31` = NULL WHERE `asset_id` = %s", $objAssetTransaction->Asset->AssetId );
	...
	protected function DisplayLabels() {
		...
		if ($this->blnEditMode) {
			$this->dtgAssetTransact->AddColumn ( new QDataGridColumn ( '&nbsp;', '<?= $_FORM->lstLocationAssetReceived_Render($_ITEM) ?> <?= $_FORM->btnReceiveAssetTransaction_Render($_ITEM) ?>', array (
					'CssClass' => "dtgcolumn",
					'HtmlEntities' => false 
			) ) );
			$this->dtgInventoryTransact->AddColumn ( new QDataGridColumn ( '&nbsp;', '<?= $_FORM->lstLocationInventoryReceived_Render($_ITEM) ?> <?= $_FORM->txtQuantityReceived_Render($_ITEM) ?> <?= $_FORM->btnReceiveInventoryTransaction_Render($_ITEM) ?>', array (
					'CssClass' => "dtgcolumn",
					'HtmlEntities' => false 
			) ) );
			$this->dtgAssetTransact->AddColumn ( new QDataGridColumn ( 'Cancel', '<?= $_FORM->btnCancelAssetTransaction_Render($_ITEM) ?>', array (
					'CssClass' => "dtgcolumn",
					'HtmlEntities' => false 
			) ) );
			$this->dtgInventoryTransact->AddColumn ( new QDataGridColumn ( 'Cancel', '<?= $_FORM->btnCancelInventoryTransaction_Render($_ITEM) ?>', array (
					'CssClass' => "dtgcolumn",
					'HtmlEntities' => false 
			) ) );
		}
	...
	protected function DisplayInputs() {
		...
		if (! $this->objReceipt->ReceivedFlag) {
			...
			$this->dtgAssetTransact->AddColumn ( new QDataGridColumn ( '&nbsp;', '<?= $_FORM->RemoveAssetColumn_Render($_ITEM) ?>', array (
					'CssClass' => "dtg_column",
					'HtmlEntities' => false 
			) ) );
			$this->dtgInventoryTransact->AddColumn ( new QDataGridColumn ( '&nbsp;', '<?= $_FORM->RemoveInventoryColumn_Render($_ITEM) ?>', array (
					'CssClass' => "dtg_column",
					'HtmlEntities' => false 
			) ) );
		} else {
