 ...
 * of the Shipment class.
 * It extends from the code-generated
...
class ShipmentEditForm extends ShipmentEditFormBase {
	...
	public $lstBox; // Container to store box value
	protected $lblBoxValue; // Store the resulting value selected
...
elseif ($intDayOfWeek == 6) {
	...
	protected function lstBox_Create() {
		$this->lblBoxValue = new QLabel ( $this );
		$this->lblBoxValue->Text = '<None>';
		...
		if ($lstBoxArray)
			foreach ( $lstBoxArray as $objValue ) {
				$this->lstBox->AddItem ( $objValue->__toString (), $objValue );
			}
		
		$this->lstBox->AddAction ( new QChangeEvent (), new QAjaxAction ( 'lstBox_Select' ) );
	}
	protected function lstBox_Select() {
		$objBox = $this->lstBox->SelectedValue;
		if ($objBox) {
			$this->lblBoxValue->Text = $objBox->__toString ();
		} else {
			$this->lblBoxValue->Text = '<None>';
		}
	}
	...
	protected function dtgAssetTransact_Create() {
		...
		$this->dtgAssetTransact->AddColumn ( new QDataGridColumn ( 'Asset Tag', '<?= $_ITEM->Asset->__toStringWithLink("bluelink") ?> <?= $_ITEM->ToStringHovertips($_CONTROL) ?>', array (
				...
				'ReverseOrderByClause' => QQ::OrderBy ( QQN::AssetTransaction ()->Asset->AssetCode, false ),
				'CssClass' => "dtg_column",
				'HtmlEntities' => false 
		) ) );
		$this->dtgAssetTransact->AddColumn ( new QDataGridColumn ( 'Model', '<?= $_ITEM->Asset->AssetModel->__toStringWithLink("bluelink") ?>', array (
				'OrderByClause' => QQ::OrderBy ( QQN::AssetTransaction ()->Asset->AssetModel->ShortDescription ),
				'ReverseOrderByClause' => QQ::OrderBy ( QQN::AssetTransaction ()->Asset->AssetModel->ShortDescription, false ),
				'Width' => "200",
				'CssClass' => "dtg_column",
				'HtmlEntities' => false 
		) ) );
		$this->dtgAssetTransact->AddColumn ( new QDataGridColumn ( 'Location', '<?= $_ITEM->SourceLocation->__toString() ?>', array (
				'CssClass' => "dtg_column" 
		) ) );
		
		if (! $this->blnEditMode) {
			$this->dtgAssetTransact->AddColumn ( new QDataGridColumn ( 'Action', '<?= $_FORM->RemoveAssetColumn_Render($_ITEM) ?>', array (
					'CssClass' => "dtg_column",
					'HtmlEntities' => false 
			) ) );
			$this->dtgAssetTransact->AddColumn ( new QDataGridColumn ( 'Advanced', '<?= $_FORM->AdvancedColumn_Render($_ITEM) ?>', array (
					'CssClass' => "dtg_column",
					'HtmlEntities' => false 
			) ) );
			$this->dtgAssetTransact->AddColumn ( new QDataGridColumn ( 'Due Date', '<?= $_FORM->DueDateColumn_Render($_ITEM) ?>', array (
					'CssClass' => "dtg_column",
					'HtmlEntities' => false 
			) ) );
		}
	...
	protected function dtgInventoryTransact_Create() {
		...
		$this->dtgInventoryTransact->AddColumn ( new QDataGridColumn ( 'Inventory Code', '<?= $_ITEM->InventoryLocation->InventoryModel->__toStringWithLink("bluelink") ?>', array (
				...
				'ReverseOrderByClause' => QQ::OrderBy ( QQN::InventoryTransaction ()->InventoryLocation->InventoryModel->InventoryModelCode, false ),
				'CssClass' => "dtg_column",
				'HtmlEntities' => false 
		) ) );
		$this->dtgInventoryTransact->AddColumn ( new QDataGridColumn ( 'Inventory Model', '<?= $_ITEM->InventoryLocation->InventoryModel->ShortDescription ?>', array (
				'OrderByClause' => QQ::OrderBy ( QQN::InventoryTransaction ()->InventoryLocation->InventoryModel->ShortDescription ),
				'ReverseOrderByClause' => QQ::OrderBy ( QQN::InventoryTransaction ()->InventoryLocation->InventoryModel->ShortDescription, false ),
				'Width' => "200",
				'CssClass' => "dtg_column" 
		) ) );
		$this->dtgInventoryTransact->AddColumn ( new QDataGridColumn ( 'Source Location', '<?= $_ITEM->SourceLocation->__toString() ?>', array (
				'CssClass' => "dtg_column" 
		) ) );
		$this->dtgInventoryTransact->AddColumn ( new QDataGridColumn ( 'Quantity', '<?= $_ITEM->Quantity ?>', array (
				'CssClass' => "dtg_column" 
		) ) );
		if (! $this->blnEditMode) {
			$this->dtgInventoryTransact->AddColumn ( new QDataGridColumn ( 'Action', '<?= $_FORM->RemoveInventoryColumn_Render($_ITEM) ?>', array (
					'CssClass' => "dtg_column",
					'HtmlEntities' => false 
			) ) );
		}
		...
		if ($this->objAssetTransactionArray) {
			foreach ( $this->objAssetTransactionArray as $objAssetTransaction ) {
				if ($objAssetTransaction->Asset instanceof Asset) {
					$arrAssetTransactions = AssetTransaction::LoadArrayByAssetId ( $objAssetTransaction->Asset->AssetId, array (
							QQ::OrderBy ( QQN::AssetTransaction ()->CreationDate, false ) 
					) );
			...
			if ($intEntityQtypeId == EntityQtype::AssetInventory || $intEntityQtypeId == EntityQtype::Asset) {
				...
				foreach ( $this->objAssetTransactionArray as $objAssetTransaction ) {
					if ($objNewerAssetTransaction = $objAssetTransaction->NewerTransaction ()) {
						...
						if ($objAssetTransaction->ScheduleReceiptFlag && $objNewerAssetTransaction->DestinationLocationId == null || $this->objShipment->ToCompanyId == $this->objShipment->FromCompanyId && $objNewerAssetTransaction->DestinationLocationId == null) {
